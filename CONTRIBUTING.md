...
| `mlclient/_client.py`, `mlclient/_manager.py` | Main client composition and environment-based client creation. |
| `mlclient/clients/` | HTTP transport, API call execution and session ownership. |
| `mlclient/http.py` | Resolved HTTP settings, cloning and session-sharing compatibility. |
| `mlclient/env.py`, `mlclient/connection.py`, `mlclient/auth.py` | Declarative connection settings, inheritance, validation and authentication. |
...
API wrappers execute requests through sync/async transport clients; Calls
themselves are synchronous request descriptions.

### Public imports and module dependencies

Export a public symbol from one canonical namespace using `__all__`. Main
clients and the manager belong to `mlclient`; transport clients to
`mlclient.clients`; settings to `mlclient.http`, `mlclient.auth`,
`mlclient.connection`, or `mlclient.env`; domain models to `mlclient.models`.
The API reference reads these exports, so update the import contract tests and
examples whenever the public surface changes. Internal helpers use `_` names
and retain docstrings explaining their parameters, behavior, and exceptions.

Implementation modules import definitions from their owning modules rather than
through the root facade. Models, Calls, authentication, and transport must not
import the main client, manager, services, or CLI. The main client composes those
lower layers; jobs and CLI may use it. Type-only imports describe real dependencies
and should follow the same direction.
...
Prefer observable behavior in tests; test private mechanisms directly when that
is the purpose of the test. Keep transport options out of MarkLogic query parameters.
...
5. Update public docstrings and the Python guide. Canonical public exports enter the
   generated API reference automatically; no reference stub is needed.
...
`docs/index.md`, `docs/quickstart.md`, `docs/user/environments.md`, the Python task guides
under `docs/user/python/`, and the command
pages under `docs/user/cli/`. Keep README examples aligned with Home. Register
new guide pages in `mkdocs.yml`; API pages are generated from canonical public exports
...
reference pages. See the [documentation contribution guide](https://github.com/monasticus/mlclient/blob/main/docs/documentation.md)
...
for the commands, publication checks and recovery procedures.
