...
``--interval`` (``-i``) sets the seconds between polls (default ``5``, between
``1`` and ``3600``):
...
In watch mode, ``--overwrite`` (``-o``) repaints the output in place instead of
scrolling the terminal, keeping the last ``--lines`` (``-l``) statuses visible
(default ``3``, between ``1`` and ``100``):
