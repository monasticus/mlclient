...
class DatabasePropertiesGetCall(ResourceCall):
    ...
    def __init__(self, database_id: str = None, database_name: str = None, data_format: str = "xml"):
        """
        Parameters
        ----------
        database_id : str
        ...
        DatabasePropertiesGetCall.__validate_params(database_id, database_name, data_format)
    ...
    def endpoint(self):
        ...
        return DatabasePropertiesGetCall.__ENDPOINT_TEMPLATE.format(self.__id if self.__id else self.__name)

    @staticmethod
    def __validate_params(database_id: str, database_name: str, data_format: str):
...
class DatabasePropertiesPutCall(ResourceCall):
    ...
    def __init__(self, database_id: str = None, database_name: str = None, body: Union[str, dict] = None):
        ...
        """
        DatabasePropertiesPutCall.__validate_params(database_id, database_name, body)
    ...
    def endpoint(self):
        ...
        return DatabasePropertiesPutCall.__ENDPOINT_TEMPLATE.format(self.__id if self.__id else self.__name)
    ...
    def __validate_params(database_id: str, database_name: str, body: Union[str, dict]):
        ...
        if not body or body is None or isinstance(body, str) and re.search("^\\s*$", body):
            endpoint = DatabasePropertiesPutCall.__ENDPOINT_TEMPLATE.format(database_id
                                                                            if database_id
                                                                            else database_name)
            raise exceptions.WrongParameters(f"No request body provided for PUT {endpoint}!")
