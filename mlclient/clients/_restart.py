...
class _RestartWaiter:
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize _RestartWaiter instance.
    ...
    def _get_last_startup_by_host(
        ...
        if not _RestartWaiter.is_restart_response(response):
            return {}

        content_type = response.headers.get(const.HEADER_NAME_CONTENT_TYPE, "")
        if "json" in content_type:
            return _RestartWaiter._get_last_startup_by_host_from_json(
                response.content,
            )
        return _RestartWaiter._get_last_startup_by_host_from_xml(
            response.content,
        )
