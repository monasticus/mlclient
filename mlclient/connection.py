...
def build_connection_auth(
    ...
    """Build the httpx.Auth handler for a resolved connection.

    A Cloud connection authenticates via its API key regardless of ``auth``,
    yielding a MarkLogicCloudAuth bound to ``base_url`` and the connection's
    verification setting. Every other connection resolves its ``auth`` through
    the auth layer. This keeps the Cloud-versus-credential branch in the
    connection layer, where the transport decides how identity is proven,
    rather than in the clients.
    ...
    base_url : str
        The connection base URL, used to reach the Cloud ``/token`` endpoint.
    ...
    """
    if connection.cloud is not None:
...
def transport_verify(connection: ConnectionMode) -> ssl.SSLContext | bool:
    ...
    disabled (httpx interprets ``verify=False`` as no checks).
