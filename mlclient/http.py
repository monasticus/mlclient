...
def _normalize_retry(retry):
    """Turn a retry argument into None or a Retry.

    None leaves the strategy unspecified, so DEFAULT_RETRY_STRATEGY applies to
    ordinary requests. An int is shorthand for the retry count, Retry(total=n);
    0 disables retries. A Retry passes through unchanged, preserving its
    identity so separately built configs still compare by object.
    """
    if retry is None or isinstance(retry, Retry):
        return retry
    return Retry(total=retry)


def _normalize_limits(limits):
    """Turn a limits argument into None or an httpx.Limits.

    None defers to httpx's own pool default. An int is shorthand for the
    concurrency cap, httpx.Limits(max_connections=n). An httpx.Limits is copied,
    isolating the configuration from caller-owned mutable settings.
    """
    if isinstance(limits, int):
        return httpx.Limits(max_connections=limits)
    return copy(limits)
...
class HTTPConfig:
    ...
    def __init__(
        ...
        self._limits = _normalize_limits(limits)
    ...
    def resolve(
        ...
        *,
        ...
        limits: httpx.Limits | int | None = None,
        timeout=UNSET,
    ) -> HTTPConfig:
        ...
        retry : Retry | int | None, default DEFAULT_RETRY_STRATEGY
            The retry strategy for transport creation. None leaves the strategy
            unspecified, using DEFAULT_RETRY_STRATEGY for ordinary requests. An
            int is shorthand for the retry count, Retry(total=n); 0 disables
            retries.
        limits : httpx.Limits | int | None, default None
            The connection-pool limits for transport creation. None leaves the
            limits unset, deferring to httpx's own default. An int is shorthand
            for the concurrency cap, httpx.Limits(max_connections=n).
        ...
        """
        connection = _resolve_connection(protocol, port, ssl, cloud)
        auth_method = _default_auth(auth, connection)
        _validate_config(connection, auth_method)
        base_url = cls._build_base_url(connection.protocol, host, connection.port)
        auth = _build_connection_auth(
            connection,
            auth_method,
            username,
            password,
    ...
    @property
    def connection(self) -> _ConnectionMode:
    ...
    def with_health_defaults(self) -> HTTPConfig:
        ...
        Retry defaults to NO_RETRY_STRATEGY and timeout to _HEALTH_TIMEOUT.
        ...
        if not self.has_explicit_timeout:
            overrides["timeout"] = _HEALTH_TIMEOUT
    ...
    def transport_verify(self) -> ssl.SSLContext | bool:
        ...
        return _transport_verify(self._connection)
