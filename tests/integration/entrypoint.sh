...
# principal is available for the kerberos-ticket app server. The data directory
# is owned by root in a fresh image, so the copy and ownership fix need sudo.
if [ -f "${SERVICE_KEYTAB}" ]; then
    sudo cp "${SERVICE_KEYTAB}" /var/opt/MarkLogic/services.keytab
    sudo chown marklogic_user:users /var/opt/MarkLogic/services.keytab
    sudo chmod 600 /var/opt/MarkLogic/services.keytab
fi
