...
# MarkLogic requires an HS256 secret exactly 256 bits long and hex-decodes the
# stored value, so the secret is 64 hex characters and the signer keys off its
# decoded bytes. A shorter or non-hex secret is rejected before signature checks.
OAUTH_SECRET = "bf425ceec67e1fbcec6f51d9b7229a10ce2b0aa5714b02340707bffab42c13b7"
...
class ManagementSession:
    ...
    def ensure_cert_user(self, common_name: str) -> None:
        ...
        response = self._request(
            "POST",
            f"{self._manage}/manage/v2/users",
            headers={"Content-Type": "application/json"},
            json={
                "user-name": common_name,
                "description": "Integration mTLS user",
                "password": "unused",
                "role": ["admin"],
            },
        )
...
xdmp:invoke-function(
  function() {
    let $_ :=
      ...
      else sec:create-role($role, "Integration OAuth role", "admin", (), (), (), $role)
