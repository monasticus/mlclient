...
def _mint_jwt(oauth_config: dict) -> str:
    ...
    payload = {
        ...
        # future exp is required for the token to validate.
        "iat": now,
        "exp": now + datetime.timedelta(hours=1),
    }
    return jwt.encode(
        payload,
        # MarkLogic hex-decodes the configured HS256 secret before computing the
        # HMAC, so the signer keys off the decoded bytes, not the hex string.
        bytes.fromhex(oauth_config["secret"]),
