...
def default_forest_properties_put_call():
    ...
    return ForestPropertiesPutCall(forest="forest-1", body={"forest-name": "custom-forest"})
...
def test_validation_body_param():
    ...
    assert err.value.args[0] == "No request body provided for PUT /manage/v2/forests/{id|name}/properties!"
...
def test_endpoint():
    ...
    assert ForestPropertiesPutCall(forest="1",
                                   body={"forest-name": "custom-forest"}).endpoint() == expected__id_endpoint
    assert ForestPropertiesPutCall(forest="forest-1",
...
def test_headers_for_dict_body():
    call = ForestPropertiesPutCall(forest="forest-1", body={"forest-name": "custom-forest"})
    assert call.headers() == {
