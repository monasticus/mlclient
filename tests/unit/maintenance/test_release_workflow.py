...
@pytest.mark.parametrize(
    "version",
    ["v1.0.0", "1.0", "01.0.0", "1.0.0-dev", "1.0.0+local", "1.0.0rc01"],
)
...
def checkout(tmp_path, monkeypatch):
    ...
    (root / "mlclient").mkdir()
    (root / "mlclient/_version.py").write_text('__version__ = "1.0.0"\n')
...
def test_tag_must_match_exported_version(checkout):
    ...
    (checkout / "mlclient/_version.py").write_text('__version__ = "0.4.1"\n')
    ...
    def run(*args):
        ...
        if args[0] == "poetry":
            if "--dry-run" in args:
                return "1.1.0"
            if args[-1] == "1.1.0":
                for file in ("pyproject.toml", "mlclient/_version.py"):
                    path = checkout / file
