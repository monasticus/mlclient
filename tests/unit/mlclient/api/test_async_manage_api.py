...
@pytest.mark.asyncio
@respx.mock
async def test_get_group_properties():
    response_body = resources_utils.read_test_resource_bytes(
        __file__,
    ...
    async with AsyncMLClient() as ml:
        resp = await ml.manage.groups.get_properties("Default", data_format="json")
...
@respx.mock
async def test_get_group_properties_forwards_timeout(timeout):
    ml_mocker = MLRespXMocker(use_router=False)
    ...
    async with AsyncMLClient() as ml:
        await ml.manage.groups.get_properties("Default", timeout=timeout)
...
@pytest.mark.asyncio
@respx.mock
async def test_put_group_properties():
    response_body = resources_utils.read_test_resource_bytes(
        __file__,
    ...
    async with AsyncMLClient() as ml:
        resp = await ml.manage.groups.put_properties(
            "non-existing-group",
...
@pytest.mark.parametrize("timeout", [None, 2, httpx.Timeout(5, read=7)])
@respx.mock
async def test_put_group_properties_forwards_timeout(timeout):
    ml_mocker = MLRespXMocker(use_router=False)
    ml_mocker.with_url("http://localhost:8002/manage/v2/groups/Default/properties")
    ml_mocker.with_request_content_type("application/json")
    ...
    async with AsyncMLClient() as ml:
        await ml.manage.groups.put_properties(
            "Default",
            {"file-log-level": "info"},
            timeout=timeout,
        )
