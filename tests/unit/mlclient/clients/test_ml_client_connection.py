...
def test_default_is_http_digest():
    ml = MLClient()
    assert ml.http.config.protocol == "http"
    assert ml.http.config.port == 8000
...
def test_https_server_cert_only():
    ...
    assert ml.http.config.ssl is not None
    assert not ml.http.config.connection.is_mutual_tls
...
def test_client_cert_infers_certificate_auth():
    ...
    assert ml.http.config.protocol == "https"
    assert ml.http.config.connection.is_mutual_tls
    assert ml.http.config.auth is None
...
def test_cloud_forces_https_443_and_internal_auth():
    ...
    assert ml.http.config.base_path == "/ml/example/manage"
...
def _cloud_probe_request(ml: MLClient) -> httpx.Request:
    ...
    url = f"{ml.http.base_url}{ml.http.config.base_path}{_PROBE_ENDPOINT}"
    with respx.mock:
