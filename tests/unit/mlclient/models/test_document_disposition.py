...
def test_from_header_for_inline():
    ...
    assert DocumentsDisposition.from_header(raw) == DocumentsDisposition(
...
def test_is_attachment_false_for_inline():
    disposition = DocumentsDisposition(type=DocumentsBodyPartType.INLINE)
...
def test_is_inline_false_for_attachment():
    disposition = DocumentsDisposition(type=DocumentsBodyPartType.ATTACHMENT)
...
def test_parse_header_part_maps_aliases_and_strips_filename_quotes():
    assert DocumentsDisposition._parse_header_part("attachment") == (
        "type_",
        "attachment",
    )
    assert DocumentsDisposition._parse_header_part('filename="/doc.xml"') == (
        "filename",
        "/doc.xml",
    )
    assert DocumentsDisposition._parse_header_part("format=json") == ("format", "json")
...
def test_to_header_for_inline():
    disposition = DocumentsDisposition(
        type=DocumentsBodyPartType.INLINE,
        extension="jpeg",
        directory="/path/to/",
        repair=Repair.NONE,
