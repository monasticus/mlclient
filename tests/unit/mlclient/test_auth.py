...
def test_kerberos_mutual_auth_warns_without_server_token(monkeypatch, caplog):
    ...
    with (
        caplog.at_level(logging.WARNING, logger="mlclient.auth"),
        contextlib.suppress(StopIteration),
    ):
        flow.send(success)

    assert any("Mutual authentication incomplete" in r.message for r in caplog.records)
