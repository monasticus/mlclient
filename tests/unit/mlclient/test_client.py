...
from mlclient import _client as ml_client_module
from mlclient.api import RestApi
...
from mlclient.http import DEFAULT_RETRY_STRATEGY, HTTPConfig
from mlclient.responses import MLResponseParser
from mlclient.services import DocumentsService, EvalService
...
@respx.mock
def test_healthcheck_defaults_to_health_timeout():
    ml_mocker = MLRespXMocker(use_router=False)
    ml_mocker.with_url("http://localhost:7997/")
    ml_mocker.with_response_code(200)
    ml_mocker.with_empty_response_body()
...
@respx.mock
def test_healthcheck_per_call_timeout_reaches_transport():
    ml_mocker = MLRespXMocker(use_router=False)
    ml_mocker.with_url("http://localhost:7997/")
    ml_mocker.with_response_code(200)
    ml_mocker.with_empty_response_body()
...
def test_wait_for_restart_builds_restart_waiter_and_uses_default_retry(
    ...
    waiter.wait_for_restart_completion.assert_called_once_with(
        ...
        timeout=12.0,
        poll_interval=0.5,
        retry=ml_client_module._RESTART_RETRY_STRATEGY,
    )
